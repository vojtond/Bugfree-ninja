...
void trojinsert(int i, string op1, string op2, string result){
    ...
    pom->data.inst=i;       /* Naplnìní prvku seznamu daty */
    pom->data.op1=op1;
    pom->data.op2=op2;
    ...
    else{               /* Jinak je novì vytvoøený prvek na prvním místì */
        Trfirst=pom;
        pozice = 1;
        pom->data.pozice=pozice;
    }
...
void trojfindpoz(int pozice){
    ...
    while (pom!=NULL){  /* Procházení celého seznamu a hledání pozice prvku */
        if (pom->data.pozice == pozice){
                Pomlabelx++;        /* Ošetøení když je v IF i ELSE vìtev */
                strInit(&pom->data.op1);
                generatelabel(pom);
                pom->data.label=0;
                pom->data.inst=42;
...
int trojfindlab(string o){
    ...
    if ((o.str[3]>='0')&&(o.str[3]<='9')){  /* Pro hodnoty vìtší jak 100 */
        int f=o.str[3];
        f-=48;
        vysl+=100*f;
    }
    else if ((o.str[2]>='0')&&(o.str[2]<='9')){ /* Pro hodnoty vìtší jak 10 */
        int f=o.str[2];
        f-=48;
        vysl+=10*f;
    }
    else if ((o.str[1]>='0')&&(o.str[1]<='9')){ /* Pro hodnoty menší jak 10 */
        int f=o.str[1];
        f-=48;
        vysl+=f;
    }
