...
#include "str.h"
int key(string *klic,string *master);/*generování klíèe*/
...
typedef struct
{
    struct  GlobTabItem *first ;/*ukazatel na koøen glob tabulky*/
}tGlobSymbolTable;

typedef struct/*øídící struktura*/
{
    int pomlog;/*jsme v hlavièce funkce*/
    int deklaration;/*poøadí argumentu*/
    string nazev_ident;/*pomocný název tokenu*/
    string attr_token;/*název tokenu*/
    string nazev_func;/*název funkce*/
    sLokTableItem *aktiv;/*koøen aktivní lokázní tabulka*/
    sGlobTableItem *aktivG;/*aktivní glob. uzel*/
    int    pocet_argumentu;/*poøadí argumnetu*/
...
void TableFree(tGlobSymbolTable *T,Tridic *ridic,sGlobTableItem *koren, int *in);/*uvolòování tabulek*/
void TableFreeLok(tGlobSymbolTable *T,Tridic *ridic,sLokTableItem *koren);/*uvolnìní lokální tabulky*/
...
void ItemFreeAktu(sGlobTableItem *pomg,sLokTableItem *poml);/*uvolnìní aktuálního prvku*/
...
void PopTopR(sRamec **Ritem);/*operace výbìr ze zásobníku*/
...
int get_token(FILE *F, double *num, string *stri);/*naètení tokenu*/
